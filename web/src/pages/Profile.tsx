...
type CropMode = {
  kind: "avatar" | "gallery-new" | "gallery-existing";
  sourceUrl: string;
  index?: number;
  fileName: string;
  mimeType: string;
...
export default function Profile() {
  ...
  const tierConfig: Record<
    NonNullable<ProfileData["campusTier"]>,
  ...
  useEffect(() => {
    Promise.all([
      api<ProfileData>("/profiles/me"),
      api("/admin/stats").then(() => true).catch(() => false),
      api<PremiumStatus>("/premium/status").catch(() => ({} as PremiumStatus)),
    ])
  ...
  const updateField =
    (field: "fullName" | "age" | "bio" | "hobbies" | "interests" | "gender" | "branch" | "year") =>
  ...
  function openCropModal(mode: CropMode) {
    setCropMode(mode);
  ...
  function closeCropModal() {
    if (cropMode?.sourceUrl.startsWith("blob:")) URL.revokeObjectURL(cropMode.sourceUrl);
    setCropMode(null);
  ...
  function clampOffset(offset: number, scaled: number, frame: number) {
  ...
  function getCropBounds(zoom = cropZoom) {
    const coverScale =
      cropMeta.width > 0 && cropMeta.height > 0
  ...
  function clampCropPosition(x: number, y: number, zoom = cropZoom) {
  ...
  useEffect(() => {
    ...
    function onTouchMove(e: TouchEvent) {
      if (!dragRef.current.active || e.touches.length === 0) return;
      const t = e.touches[0];
  ...
  useEffect(() => {
    ...
    if (next.x !== cropX) setCropX(next.x);
    if (next.y !== cropY) setCropY(next.y);
  ...
  async function openViewerFromUrl(url: string, kind: "avatar" | "gallery-existing", index?: number) {
    try {
  ...
  async function createCroppedBlob(): Promise<Blob | null> {
    if (!cropImageEl || !cropMeta.width || !cropMeta.height) return null;
  ...
  async function uploadGalleryBlob(blob: Blob) {
    const file = new File([blob], `gallery-${Date.now()}.jpg`, { type: blob.type || "image/jpeg" });
    const formData = new FormData();
    formData.append("photo", file);
    return api<{ photos: string[] }>("/profiles/me/photos", {
      method: "POST",
      body: formData,
    });
  }
  ...
  async function saveCroppedImage() {
    ...
    try {
      ...
      if (!blob) throw new Error("Invalid crop");
      ...
      if (cropMode.kind === "avatar") {
        ...
        setUploadingGallery(true);
        await api<{ photos: string[] }>(`/profiles/me/photos/${cropMode.index}`, { method: "DELETE" });
        const data = await uploadGalleryBlob(blob);
        setForm((prev) => ({ ...prev, photos: data.photos ?? prev.photos ?? [] }));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const refreshed = await api<ProfileData>("/profiles/me");
      setForm((prev) => ({
        ...prev,
  ...
  async function activateBoost() {
    try {
      const response = await api<{ profileBoostUntil?: string; boostRemainingSeconds?: number }>(
        "/premium/boost",
        { method: "POST" }
      );
      setForm((prev) => ({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-200 to-purple-200 py-6 px-3 sm:px-6">
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-4 items-start">
        <aside className="rounded-3xl bg-rose-50/90 border border-white/70 shadow-md p-4 self-start">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="text-2xl font-black text-slate-900 truncate">{form.fullName}</p>
              <p className="text-base text-slate-600">Age {form.age || "--"}</p>
            </div>
          </div>

          {Number(form.id ?? 0) > 0 && Number(form.id ?? 0) <= 100 && (
            <p className="mt-3 inline-flex rounded-full px-2.5 py-1 text-xs font-semibold bg-amber-100 border border-amber-200 text-amber-700">
              Early Adopter
            </p>
          )}
          ...
          <button
            ...
            disabled={uploading}
            className="mt-3 w-full rounded-xl bg-pink-600 text-white px-3 py-2 text-sm font-semibold hover:bg-pink-700 disabled:opacity-60"
          >
            {uploading ? "Uploading..." : "Upload photo"}
          </button>

          <div className="mt-2 grid grid-cols-3 gap-2">
            {(form.photos ?? []).slice(0, 6).map((photo, idx) => (
              <div key={`${photo}-${idx}`} className="relative rounded-xl overflow-hidden border border-slate-200 bg-white">
                <img
                  src={photo}
                  alt={`Gallery ${idx + 1}`}
                  className="w-full h-20 object-cover cursor-zoom-in"
                  onClick={() => openViewerFromUrl(photo, "gallery-existing", idx)}
                />
                <button
                  type="button"
                  onClick={async () => {
                    try {
                      const data = await api<{ photos: string[] }>(`/profiles/me/photos/${idx}`, { method: "DELETE" });
                      setForm((prev) => ({ ...prev, photos: data.photos ?? [] }));
                    } catch {
                      setMessage({ type: "err", text: "Could not remove photo" });
                    }
                  }}
                  className="absolute top-1 right-1 w-5 h-5 rounded-full bg-black/60 text-white text-[11px]"
                >
                  ×
                </button>
              </div>
            ...
            {(form.photos?.length ?? 0) < 6 && (
              <button
                ...
                className="h-20 rounded-xl border border-dashed border-slate-300 bg-white text-slate-500 text-xs font-semibold hover:bg-slate-50"
              >
                {uploadingGallery ? "Adding..." : "+ Add"}
              </button>
            )}
          </div>
          ...
          <div className="mt-4 space-y-2">
            ...
            <div
              className={`rounded-xl px-3 py-2 text-sm font-semibold border text-center ${
                form.isPremium ? "bg-amber-100 text-amber-700 border-amber-200" : "bg-slate-100 text-slate-600 border-slate-200"
              }`}
            >
              {form.isPremium ? "Premium Active" : "Free Plan"}
            </div>
          </div>

          {boostCountdown && (
            <p className="mt-3 text-xs rounded-lg border border-fuchsia-100 bg-fuchsia-50 px-2 py-1.5 text-fuchsia-700">
              Boost active: {boostCountdown}
            </p>
          )}

          {hasAdminAccess(form.isAdmin) && (
            <Link
              to="/internal/admin?section=overview"
              className="mt-3 inline-flex w-full items-center justify-center rounded-xl bg-slate-900 text-white py-2.5 text-sm font-semibold hover:bg-slate-800"
            >
              Admin Dashboard
            </Link>
          )}
        ...
        <section className="rounded-3xl bg-rose-50/90 border border-white/70 shadow-md p-4 sm:p-6 self-start transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5">
          <h1 className="text-3xl font-black text-slate-900">Date Profile</h1>
          ...
          <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50/70 p-3">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-500 font-semibold">My Basics</p>
            <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-2">
              {[
                ...
                <div key={item.k} className="rounded-xl border border-slate-200 bg-white px-3 py-2">
                  <p className="text-xs text-slate-500">{item.k}</p>
                  <p className="text-sm font-semibold text-slate-800 truncate">{String(item.v)}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="mt-5 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <input className="rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="Full name" value={form.fullName} onChange={updateField("fullName")} />
              <input className="rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="Age" type="number" min={18} value={form.age} onChange={updateField("age")} />
              <input className="rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="Gender" value={form.gender ?? ""} onChange={updateField("gender")} />
              <input className="rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="Branch" value={form.branch ?? ""} onChange={updateField("branch")} />
              <input className="rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="Year" type="number" value={form.year ?? ""} onChange={updateField("year")} />
              <input className="rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="Interests" value={form.interests ?? ""} onChange={updateField("interests")} />
            </div>
            <textarea className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" rows={3} placeholder="Bio" value={form.bio ?? ""} onChange={updateField("bio")} />
            <input className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" placeholder="Hobbies" value={form.hobbies ?? ""} onChange={updateField("hobbies")} />
            <button
              type="submit"
              disabled={saving}
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="rounded-xl border border-violet-100 bg-violet-50 p-3">
              <p className="text-xs uppercase tracking-[0.2em] text-violet-700">Campus Level</p>
              <div className="mt-2 flex items-center justify-between">
                <span className="rounded-full bg-white border border-violet-200 px-2.5 py-1 text-xs font-semibold text-violet-700">
                  {currentTier}
                </span>
                <span className="text-sm font-bold text-slate-800">{score} pts</span>
              </div>
              <div className="mt-2 h-2 rounded-full bg-violet-100 overflow-hidden">
                <div className="h-full bg-gradient-to-r from-violet-500 to-fuchsia-500" style={{ width: `${progressPercent}%` }} />
              </div>
              <p className="mt-1 text-xs text-slate-600">
                {nextTarget == null ? "Top tier unlocked." : `Next tier target: ${nextTarget} pts`}
              </p>
            </div>

            <div className="rounded-xl border border-amber-100 bg-amber-50 p-3">
              <p className="text-xs uppercase tracking-[0.2em] text-amber-700">Reputation</p>
              <div className="mt-2 grid grid-cols-2 gap-2 text-sm">
                <div className="rounded-lg bg-white border border-amber-100 p-2">
                  <p className="text-slate-500">Rating</p>
                  <p className="font-bold">{typeof form.avgHostRating === "number" ? `${form.avgHostRating.toFixed(1)} / 5` : "N/A"}</p>
                </div>
                <div className="rounded-lg bg-white border border-amber-100 p-2">
                  <p className="text-slate-500">Reliability</p>
                  <p className="font-bold">{form.reliabilityScore ?? 0}%</p>
                </div>
                <div className="rounded-lg bg-white border border-amber-100 p-2 col-span-2">
                  <p className="text-slate-500">Joined / Attended / Hosted</p>
                  <p className="font-bold">
                    {form.plansJoined ?? 0} / {form.plansAttended ?? 0} / {form.plansHosted ?? 0}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="mt-3 rounded-xl border border-pink-100 bg-pink-50 p-3">
            <p className="text-xs uppercase tracking-[0.2em] text-pink-700">Invite Friends</p>
            <p className="mt-1 text-xs text-slate-600">Invite count: {form.inviteCount ?? 0}</p>
            <div className="mt-2 flex items-center gap-2">
              <input
                type="text"
                readOnly
                value={inviteLink}
                className="flex-1 rounded-lg border border-pink-100 bg-white px-2 py-1.5 text-xs text-slate-700"
              />
              <button
                type="button"
                onClick={copyInviteLink}
                className="rounded-lg bg-pink-600 text-white text-xs font-semibold px-3 py-1.5 hover:bg-pink-700"
              >
                Copy
              </button>
            </div>
            {copyStatus && <p className="mt-1 text-xs text-pink-700">{copyStatus}</p>}
          </div>
          <Link
            to="/feedback"
            className="mt-3 inline-flex w-full items-center justify-center rounded-xl bg-white border border-pink-200 text-pink-700 py-2.5 text-sm font-semibold hover:bg-pink-50"
          >
            Feedback
          </Link>
        </section>
      ...
      {cropMode && (
        <div className="fixed inset-0 z-[60] bg-slate-950/75 backdrop-blur-sm px-3 py-6 flex items-center justify-center">
          <div className="w-full max-w-3xl rounded-2xl border border-white/20 bg-white p-4 sm:p-5 shadow-2xl">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-lg font-bold text-slate-900">
                {isCropEditing ? "Crop photo" : "Photo preview"}
              </h2>
              <button
                type="button"
            ...
            <div className="mt-4 grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-4">
              <div className="mx-auto">
                {isCropEditing ? (
                  <div
                    ref={cropSurfaceRef}
                    className="w-[320px] h-[320px] sm:w-[360px] sm:h-[360px] rounded-2xl overflow-hidden border border-slate-200 bg-slate-100 relative cursor-grab active:cursor-grabbing select-none touch-none"
                  ...
                  >
                    ...
                    <div className="absolute inset-0 pointer-events-none">
                      ...
                      <div className="absolute left-1/3 top-0 bottom-0 w-px bg-white/45" />
                      <div className="absolute left-2/3 top-0 bottom-0 w-px bg-white/45" />
                      <div className="absolute top-1/3 left-0 right-0 h-px bg-white/45" />
                      <div className="absolute top-2/3 left-0 right-0 h-px bg-white/45" />
                    </div>
                  </div>
                ) : (
                  <img
                    src={cropMode.sourceUrl}
                    alt="Full preview"
                    className="w-[320px] h-[320px] sm:w-[360px] sm:h-[360px] rounded-2xl border border-slate-200 bg-slate-100 object-contain"
                  />
                )}
              </div>
                ...
                {isCropEditing ? (
                  <>
                    <p className="text-sm text-slate-600">
                      Drag image to position. Use mouse wheel or slider to zoom.
                    </p>
                    <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Move range: X {Math.round(-maxMoveX)} to {Math.round(maxMoveX)}, Y {Math.round(-maxMoveY)} to {Math.round(maxMoveY)}
                    </p>
                  </>
                ) : (
                  <p className="text-sm text-slate-600">
                    Click crop to fit this image properly inside your profile frame.
                  </p>
                )}
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  {!isCropEditing && (
                    <button
                      ...
                      className="rounded-lg bg-slate-800 text-white px-3.5 py-2 text-sm font-semibold hover:bg-slate-900"
                    >
                      Crop image
                    </button>
                  )}
                  {isCropEditing && (
                    <button
                      ...
                      disabled={savingCrop}
                      className="rounded-lg bg-pink-600 text-white px-3.5 py-2 text-sm font-semibold hover:bg-pink-700 disabled:opacity-60"
                    >

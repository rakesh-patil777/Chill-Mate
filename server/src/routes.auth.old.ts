...
router.post('/register', async (req, res) => {
	...
	if (!isValidCollegeId(collegeId)) return res.status(400).json({ error: 'Invalid college ID' });
...
router.post('/login', async (req, res) => {
	...
	const user = db.prepare(`SELECT id, passwordHash FROM users WHERE collegeId = ?`).get(collegeId) as
		| { id: number; passwordHash: string } | undefined;

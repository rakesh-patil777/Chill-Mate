...
function getReputationStats(userId) {
    ...
    const reliabilityScore = plansJoined > 0 ? Math.round((plansAttended / plansJoined) * 100) : 0;
    ...
    return {
        ...
        noShows,
        reliabilityScore,
        avgHostRating: typeof avgHostRatingRaw.avgRating === "number"
            ? Number(avgHostRatingRaw.avgRating.toFixed(2))
            : null,
    };
}
function getCampusLevel(input) {
    const scoreRaw = Number(input.plansHosted ?? 0) * 5 +
    ...
    let campusTier = "Freshman";
    if (campusScore >= 500)
        campusTier = "Campus Icon";
    else if (campusScore >= 250)
        campusTier = "Influencer";
    else if (campusScore >= 120)
        campusTier = "Connector";
    else if (campusScore >= 50)
        campusTier = "Explorer";
...
router.get("/discover", auth, (req, res) => {
    ...
    const args = [userId, userId, userId, userId];
    ...
    try {
        const list = db
            .prepare(`SELECT u.id, u.fullName, u.age, u.gender, p.bio, p.hobbies, p.interests, p.branch, p.year, p.avatarUrl
                , u.swipeStreak
         FROM users u
...
router.get("/me", auth, (req, res) => {
    ...
    try {
        const user = db
            .prepare("SELECT id, collegeId, fullName, age, gender, isAdmin, premiumUntil, profileBoostUntil, inviteCount, plansHosted, reputationBoost, swipeStreak, campusStreak FROM users WHERE id = ?")
            .get(userId);
        if (!user)
        ...
        const isPremium = Boolean(user.premiumUntil) && Date.parse(String(user.premiumUntil)) > Date.now();
...
router.post("/me/avatar", auth, upload.single("avatar"), (req, res) => {
    ...
    try {
        ...
        if (exists)
            db.prepare("UPDATE profiles SET avatarUrl = ? WHERE userId = ?").run(avatarUrl, userId);
        else
            db.prepare("INSERT INTO profiles (userId, avatarUrl) VALUES (?, ?)").run(userId, avatarUrl);
        res.json({ avatarUrl });
    }
    catch (err) {
...
router.post("/me/photos", auth, upload.single("photo"), (req, res) => {
    if (!req.file)
        return res.status(400).json({ error: "No file uploaded" });
    const userId = req.user.id;
...
router.delete("/me/photos/:index", auth, (req, res) => {
    const userId = req.user.id;
...
router.put("/me", auth, (req, res) => {
    ...
    try {
        if (fullName != null)
            db.prepare("UPDATE users SET fullName = ? WHERE id = ?").run(fullName, userId);
        ...
        if (prof) {
            ...
            if (photos !== undefined) {
                const normalized = Array.isArray(photos)
                    ? photos.filter((item) => typeof item === "string" && item.trim().length > 0).slice(0, 6)
                    : [];
                updates.push("photos = ?");
            ...
            db.prepare("INSERT INTO profiles (userId, bio, hobbies, interests, branch, year, avatarUrl, photos) VALUES (?, ?, ?, ?, ?, ?, ?, ?)").run(userId, bio ?? "", hobbies ?? "", interests ?? "", branch ?? "", year ?? null, avatarUrl ?? null, JSON.stringify(Array.isArray(photos)
                ? photos.filter((item) => typeof item === "string" && item.trim().length > 0).slice(0, 6)
                : []));
        }
        res.json({ ok: true });
